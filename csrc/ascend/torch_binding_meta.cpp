...
TORCH_LIBRARY_IMPL_EXPAND(CONCAT(_C, _ascend), Meta, ops) {
    // causal_conv1d_310
    ops.impl("npu_causal_conv1d_310", &vllm_fl::meta::npu_causal_conv1d_310_meta);
...
TORCH_LIBRARY_IMPL_EXPAND(CONCAT(_C, _ascend), Meta, ops) {
    ...
    ops.impl("npu_gemma_rms_norm", &vllm_fl::meta::npu_gemma_rms_norm_meta);
    // recurrent_gated_delta_rule meta implementation
    ops.impl("npu_recurrent_gated_delta_rule", &vllm_fl::meta::npu_recurrent_gated_delta_rule_meta);
    // Launch host print from device
    ops.impl("device_print", &vllm_fl::meta::device_print_meta);
    // launch host print from device for tensors
    ops.impl("device_print_tensor", &vllm_fl::meta::device_print_tensor_meta);
#ifdef VLLM_ENABLE_ATB_AND_DIRECT_KERNELS
    // Direct kernel meta implementations
    // Bgmv expand
    ops.impl("bgmv_expand", &vllm_fl::meta::bgmv_expand_meta);
    // Sgmv expand
    ops.impl("sgmv_expand", &vllm_fl::meta::sgmv_expand_meta);
    // MLA preprocess
    ops.impl("mla_preprocess", &vllm_fl::meta::mla_preprocess);
    ...
    // Moe_gating_top_k
    ops.impl("moe_gating_top_k", &vllm_fl::meta::moe_gating_top_k_meta);
    // Add_Rms_Norm_Bias
    ...
    ops.impl("npu_kv_quant_sparse_attn_sharedkv", &vllm_fl::meta::npu_kv_quant_sparse_attn_sharedkv_meta);
    ops.impl("npu_kv_quant_sparse_attn_sharedkv_metadata",
             &vllm_fl::meta::npu_kv_quant_sparse_attn_sharedkv_metadata_meta);
